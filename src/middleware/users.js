  ...
  body()
    .custom((payload) => {
      if (!typeof payload == Object) {
        throw new Error("input data must be json object");
      }
      const keys = Object.keys(payload).length;
      if (keys !== 4) {
        throw new Error("login data must have exactly 4 fields");
      }
      if (
        !payload.email ||
        !payload.password ||
        ...
        throw new Error(
          "input data must have email, password, name and role fields"
        );
      }
      return true;
    })
